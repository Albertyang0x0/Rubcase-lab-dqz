...
bool IxIndexHandle::insert_entry(const char *key, const Rid &value, Transaction *transaction) {
    ...
    // 3. 如果结点已满，分裂结点，并把新结点的相关信息插入父节点
        ...
        if (leaf->GetPageNo() == file_hdr_.last_leaf) file_hdr_.last_leaf = new_node->GetPageNo();
...
bool IxIndexHandle::CoalesceOrRedistribute(IxNodeHandle *node, Transaction *transaction) {
    ...
    // 4. 如果node结点和兄弟结点的键值对数量之和，能够支撑两个B+树结点,则只需要重新分配键值对
...
void IxIndexHandle::Redistribute(IxNodeHandle *neighbor_node, IxNodeHandle *node, IxNodeHandle *parent, int index) {
    ...
    // 2. 从neighbor_node中移动一个键值对到node结点中
    if (index != 0) {
        // 如果neighbor_node是node的前驱结点，在node开头插入neighbor_node的最后一个键值对
        ...
        // 如果neighbor_node是node的后继结点，在node末尾插入neighbor_node的第一个键值对
...
bool IxIndexHandle::Coalesce(IxNodeHandle **neighbor_node, IxNodeHandle **node, IxNodeHandle **parent, int index,
    ...
    for (int i = before_num; i < after_num; ++i) maintain_child(*neighbor_node, i);

    // 3. 释放和删除node结点，并删除parent中node结点的信息，返回parent是否需要被删除
    if ((*node)->GetPageNo() == file_hdr_.last_leaf) file_hdr_.last_leaf = (*neighbor_node)->GetPageNo();
