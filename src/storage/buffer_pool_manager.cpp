...
bool BufferPoolManager::FindVictimPage(frame_id_t *frame_id) {
    ...
    if (!free_list_.empty()) {
        // 1.1 未满，获得一个 frame
        *frame_id = free_list_.front();  // 获取第一个元素
        free_list_.pop_front();
        return true;
    } else {
        // 1.2 已满，使用 lru_replacer 中的方法选择淘汰页面
...
void BufferPoolManager::UpdatePage(Page *page, PageId new_page_id, frame_id_t new_frame_id) {
    ...
    if (new_page_id.page_no != INVALID_PAGE_ID) page_table_.insert(std::make_pair(new_page_id, new_frame_id));
    // 3 重置page的data，更新page id
    page->ResetMemory();
...
Page *BufferPoolManager::FetchPage(PageId page_id) {
    ...
    if (it != page_table_.end()) {  // 如果在缓冲池中存在
        frame_id_t i = it->second;  // 获取 frame_id
        replacer_->Pin(i);
        pages_[i].pin_count_++;
        return &pages_[i];
    }
    ...
    if (!FindVictimPage(&frame_id))  // 如果在淘汰页中未找到，返回 nullptr
        return nullptr;
...
bool BufferPoolManager::UnpinPage(PageId page_id, bool is_dirty) {
    ...
    // 0. 在页表中查找页面 P
    auto it = page_table_.find(page_id);
    if (it == page_table_.end())  // 如果不在缓冲池中
        return false;

    frame_id_t frame_id = it->second;
    Page *page = &pages_[frame_id];

    // 1.2 如果 P 在页表中存在，减少一次固定 (pin_count)
...
Page *BufferPoolManager::NewPage(PageId *page_id) {
    ...
    // 1. 确保调用 DiskManager::AllocatePage!
    page_id->page_no = disk_manager_->AllocatePage(page_id->fd);

    // 2. 如果缓冲池中的所有页面都被固定，返回 nullptr。
    ...
    // P 存在，更新 P 的元数据，将其添加到页面表，并将 pin_count 设置为 1。
    Page *page = &pages_[frame_id];
...
bool BufferPoolManager::DeletePage(PageId page_id) {
    ...
    // 1. 确保调用 DiskManager::DeallocatePage!
    disk_manager_->DeallocatePage(page_id.page_no);

    // 2. 在页面表中查找请求的页面 (P)。
    auto it = page_table_.find(page_id);

    // 2.1 如果 P 不存在，返回 true。
    ...
    // 2.2 如果 P 存在，但 pin-count 不为零，返回 false。有其他地方正在使用该页面。
    if (page->pin_count_ > 0) {
        return false;
    }

    // 3. 否则，可以删除 P。 从页面表中移除 P，重置其元数据，并将其返回到空闲列表。
