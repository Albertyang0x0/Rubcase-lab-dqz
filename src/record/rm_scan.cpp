...

        {
...

            {
                int i = Bitmap::first_bit(1, file_handle_->fetch_page_handle(pageno).bitmap,
                                          file_handle_->file_hdr_.num_records_per_page);  // 找到第一个包含记录的插槽
...
void RmScan::next() {
...

    {
        int i = Bitmap::next_bit(1, file_handle_->fetch_page_handle(pageno).bitmap,
                                 file_handle_->file_hdr_.num_records_per_page, slotno);  // 查找下一个包含记录的插槽

        if (i ==
            file_handle_->file_hdr_
                .num_records_per_page)  // 如果i等于页面的最大插槽数，说明没有找到下一个包含记录的插槽，将slotno设置为-1并继续下一轮查找
